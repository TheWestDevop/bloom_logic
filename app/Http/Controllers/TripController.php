...
//            }
    ...
    public function find_requests(Request $request){
        ...
        $trip_requests=TripRequest::where('user_id','!=',$driver_id)->where('from','LIKE',"%$from%")->where('destination','LIKE',"%$destination%")->where('wait_time','>=',$date)->where('status',1)->where('private_trip',"==",$type)->get();
